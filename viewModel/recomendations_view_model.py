...
class RecommendationsViewModel:
    ...
    def __filter(self) -> None:
        ...
        if self.__form_result['pc_or_laptop'] == 1: self.__pc_filter = PcFilter(self.__pc_filter.get_pcs())
        else: self.__pc_filter = PcFilter(self.__pc_filter.get_laptops())
        
        if self.__form_result['is_for_gaming'] == 1: self.__pc_filter = PcFilter(self.__pc_filter.get_gaming_pcs())
        elif self.__form_result['is_for_montage'] == 1: self.__pc_filter = PcFilter(self.__pc_filter.get_montage_pcs())
        elif self.__form_result['is_for_programming'] == 1: self.__pc_filter = PcFilter(self.__pc_filter.get_programming_pcs())
        elif self.__form_result['is_for_office'] == 1: self.__pc_filter = PcFilter(self.__pc_filter.get_office_pcs())
